...
void syncDevice (void) {
    ...
    cudaDeviceSynchronize_t cudaSyncDevice = (cudaDeviceSynchronize_t) ((uint64_t) dlsym(lib, "cudaDeviceSynchronize"));
	if(cudaSyncDevice == NULL) {
		dlclose(lib);
		cerr << "Error loading cudaDeviceSynchronize: CUDA RT function not found."
				<< endl;
		return;
	}

	std::cout << "Syncing device: " << cudaSyncDevice() << std::endl;
	dlclose(libcuda);
	dlclose (lib);
}
...
void CudaRtHandler::RegisterFatBinary(std::string& handler, void ** fatCubinHandle) {
    ...
    cout << "Registered FatBinary " << fatCubinHandle << " with handler " << handler.c_str() << endl;
}
